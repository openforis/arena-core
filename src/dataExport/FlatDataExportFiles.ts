...
type FieldsExtractor = (params: {
  survey: Survey
  columnNodeDef: ColumnNodeDef
  includeCategoryItemsLabels: boolean
  expandCategoryItems: boolean
}) => Array<{ name: string; type?: string } | string>

const fieldsExtractorByNodeDefType: Partial<Record<NodeDefType, FieldsExtractor>> = {
  [NodeDefType.code]: ({ survey, columnNodeDef, includeCategoryItemsLabels, expandCategoryItems }) => {
  ...
  [NodeDefType.coordinate]: ({ columnNodeDef }) => {
...
const getCsvExportFields = ({
  ...
  if (includeInternalUuids) {
    Arrays.addItem(viewDataNodeDef.columnParentUuidName, { excludeNulls: true })(fields)
  ...
  fields.push(
    ...nodeDefCols.flatMap((nodeDefCol) => {
      ...
      const fieldsExtractor = fieldsExtractorByNodeDefType[NodeDefs.getType(nodeDefCol)]
      if (fieldsExtractor) {
...
const getCsvObjectTransformerNullsToEmpty = () => (obj: any) => {
