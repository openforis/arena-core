...
const getNodeDefChildrenUuidsSortedByLayout = (params: {
  ...
  const childrenEntitiesInOwnPageSortedUudis = NodeDefs.getChildrenEntitiesInOwnPageUudis(cycle)(entityDef) ?? []
  ...
  if (layoutChildren.length === 0 && childrenEntitiesInOwnPageSortedUudis.length === 0) {
  ...
  const missingChildrenUuidsInLayout = childrenUuids.filter((childUuid) => {
    ...
    return (
      ...
      !childrenEntitiesInOwnPageSortedUudis.includes(childUuid) &&
  ...
  return (
    sortedChildrenDefsInSamePageUuids
      ...
      .concat(childrenEntitiesInOwnPageSortedUudis)
  )
}
