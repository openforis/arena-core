...
describe('RecordExpressionEvaluator', () => {
  ...
  test('numberToText(value) uses the language passed in the expression context', async () => {
    ...
    expect(await evaluator.evaluate('numberToText(cluster_id)', contextEn)).toBe('twelve')

    const contextFr: RecordExpressionContext = { ...contextEn, lang: LanguageCode.fr }
    expect(await evaluator.evaluate('numberToText(cluster_id)', contextFr)).toBe('douze')
  })
  ...
  const queries: Query[] = [
    ...
    // numberToText (no lang in context => falls back to English)
    { expression: 'numberToText(cluster_id)', result: 'twelve' },
    { expression: 'numberToText(null)', result: null },
    // count
