...
describe('RecordUpdater - node create', () => {
  ...
  test('Multiple entity minCount is not auto-generated by default', async () => {
    const survey = await new SurveyBuilder(
      user,
      entityDef(
        'root_entity',
        integerDef('identifier').key(),
        entityDef('plot', integerDef('plot_id').key()).multiple().minCount('2')
      )
    ).build()
    ...
    const plot0 = TestUtils.findNodeByPath({ survey, record, path: 'root_entity.plot[0]' })
    expect(plot0).toBeUndefined()
  })

  test('Multiple entity minCount is auto-generated when enabled, also for nested child multiples when parent is created', async () => {
    const survey = await new SurveyBuilder(
      user,
      entityDef(
        'root_entity',
        ...
        entityDef(
          ...
          entityDef('plot', integerDef('plot_id').key()).multiple().minCount('2').autoCreateMinCountItems()
        )
          .multiple()
          .minCount('1')
    ...
    const cluster0 = TestUtils.getNodeByPath({ survey, record, path: 'root_entity.cluster[0]' })
    expect(cluster0).not.toBeNull()

    const plot0 = TestUtils.getNodeByPath({ survey, record, path: 'root_entity.cluster[0].plot[0]' })
    const plot1 = TestUtils.getNodeByPath({ survey, record, path: 'root_entity.cluster[0].plot[1]' })
    expect(plot0).not.toBeNull()
    expect(plot1).not.toBeNull()

    const plot2 = TestUtils.findNodeByPath({ survey, record, path: 'root_entity.cluster[0].plot[2]' })
    expect(plot2).toBeUndefined()
  })
})
