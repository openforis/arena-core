...
test('ExpectedCategoryItem === CategoryItem', () => {
  ...
  expect(categoryItem.levelUuid).toBe(categoryItemParams.levelUuid)

  expect(categoryItem).toHaveProperty('parentUuid')
  expect(categoryItem.parentUuid).toBe(categoryItemParams.parentUuid)
  ...
  expect(categoryItem.props.code).toBe(categoryItemParams.props?.code)
  expect(categoryItem.props.labels).toBeTruthy()
  expect(categoryItem.props.labels).toHaveProperty(LanguageCode.en)
  expect(categoryItem.props.labels?.en).toBe(categoryItemParams.props?.labels?.en)
})
